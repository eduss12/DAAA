        ...
        group.notify(queue: .main) {
            ...
            for (id, prices) in priceData {
                guard let usdPrice = prices["usd"] as? Double,
                      ...
                      let symbol = details["symbol"] as? String else {
                    print("Skipping id: \(id) due to missing data")
                    continue
                }

                // Proveer valores predeterminados para cambio24h y volumen
                let change24h = details["price_change_24h"] as? Double ?? 0.0
                let volumen = details["total_volume"] as? Double ?? 0.0

                print("Processing: \(name) - Change: \(change24h), Volume: \(volumen)")
                ...
                let entity = Moneda(
                    ...
                    priceEUR: eurPrice,
                    lastUpdated: Date(),
                    imagen: details["image"] as? String ?? "", // Proveer valor predeterminado
                    cambio24h: change24h,
                    volumen: volumen
                )
